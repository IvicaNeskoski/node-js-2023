...
const firstLetterUppercase = (text) => {
  ...
  //   let ostatok = text.slice(1, text.lenght);
  // returnText = zgolemenaPrvaBukva + ostatok;
  // return returnText;
  return text.charAt(0).toUpperCase() + text.slice(1, text.lenght);
};
...
const textStats = (text) => {
  ...
  for (let i = 0; i < zborovi.length; i++) {
    if (zborovi[i].length === 5) output.ednakvoNa5++;
    if (zborovi[i].length > 5) output.povekeOd5++;
    if (zborovi[i].length < 5) output.pomalkuOd5++;
  }
  return output;
};
